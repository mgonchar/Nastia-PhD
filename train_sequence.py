...
def succeed_to_train():
    ...
    if global_idx == 1 and train_type_idx == 1:
        ...
        reset_input()
        w = Label(root, text="Press "+training_pattern[global_idx]+" by "+training_type[train_type_idx], bg="black", fg="white", width=root.winfo_screenwidth(), height=root.winfo_screenheight())
        w.pack()
...
def keyPress(e):
    ...
    if  sym == 'Escape':
        root.quit()
    elif sym == 'Shift_L' or '5':
        handleInput('l', time)
    elif sym == 'Shift_R' or '6':
        handleInput('r', time)
    ...
    if len(current_input) < 8:        
        ...
        if shifted_training_pattern == current_input:
            ...
            if delta > 2000:
                ...
                handleInput(rl, time)
                return
            current_delays = current_delays[1:].append(delta)
            input_idx+=1
        else: 
            reset_input()
            return
...
input_reset_timer = Timer(2.0, reset_input)
timer_started = False
